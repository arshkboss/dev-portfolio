...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3 bg-background/50 border border-border py-1 px-1 backdrop-blur-md rounded-full shadow-md">
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "relative cursor-pointer text-sm font-semibold px-3 sm:px-6 py-2 rounded-full transition-all back duration-300",
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="lamp"
                    className="absolute inset-0 w-full bg-primary/5 rounded-full -z-10 outline-1"
                    initial={false}
                    transition={{
                      type: "spring",
                      stiffness: 300,
